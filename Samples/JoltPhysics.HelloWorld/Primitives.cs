...
namespace JoltHelloWorld
{
    ...
    {
        public static void Sphere(float diameter, int tessellation, out List<VertexPositionNormalTangentTexture> vertexData, out List<ushort> indexData, bool uMirror = false, bool vMirror = false)
        {
            ...
            uIncrement *= uMirror ? 1 : -1;
            vIncrement *= vMirror ? 1 : -1;
        ...
        public static void Cube(float size, out List<VertexPositionNormalTangentTexture> vertexData, out List<ushort> indexData, bool uMirror = false, bool vMirror = false, float uTileFactor = 1, float vTileFactor = 1)
        {
            vertexData = new List<VertexPositionNormalTangentTexture>();
            indexData = new List<ushort>();

            float uCoordMin = uMirror ? uTileFactor : 0;
            float uCoordMax = uMirror ? 0 : uTileFactor;
            float vCoordMin = vMirror ? vTileFactor : 0;
            float vCoordMax = vMirror ? 0 : vTileFactor;
